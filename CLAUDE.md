...
| `/backups`, `/backups/<ts>/<file>` | List and download the hunt CSV archives from S3 (token-gated; CSV allowlist only, never the SQL dumps). |
...
- **Admin endpoint gating:** the same internal token gates the destructive/admin API endpoints (`POST /config`, `POST /rbac/*`, `/deleteuser`, `/deletepuzzle`, `POST /migrate/*`, `/newusers`, `/google/users`, `/backups*`) via the `admin_token_gated` decorator. `ADMIN_TOKEN_ENFORCE=true` in the config table enforces 403; false/absent logs a SEV2 warning and allows (rollout mode). Production runs enforced. PHP attaches the token only from puzztech-gated code paths (`*api_internal` helpers in `puzzlebosslib.php`).
...
- **Hunt archives:** `GET /backups*` expose the two CSV files (`puzzle_view.csv`, `activity.csv`) that `reset-hunt.py` writes to `s3://puzzleboss-hunt-backups/db/<timestamp>/`. The full SQL dumps sit in the same prefix and contain `SERVICE_ACCOUNT_JSON` (the Workspace service account key with Domain-Wide Delegation) plus every other config secret, so they must never be reachable from the web tier. Three independent gates enforce that: the ECS task role is scoped to `db/*/*.csv.gz`, `DOWNLOADABLE_BACKUP_FILES` in pbrest allowlists the two filenames, and `www/backupdownload.php` allowlists them again after its puzztech check. Keep all three in step; guarded by `tests/test_backup_archives.py`.
