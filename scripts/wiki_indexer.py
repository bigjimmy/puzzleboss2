...
def get_all_wiki_pages(wiki_url, exclude_prefixes=None):
    ...
    while True:
        ...
        for page in data.get("query", {}).get("allpages", []):
            ...
            pages.append({"pageid": page["pageid"], "title": title})
...
def chunk_content(title, content, chunk_size=1000, overlap=200):
    ...
    if len(content) <= chunk_size:
        return [{"title": title, "content": content, "chunk_index": 0}]
    ...
    while start < len(content):
        ...
        if chunk_text:
            chunks.append(
                {"title": title, "content": chunk_text, "chunk_index": chunk_index}
            )
            chunk_index += 1
...
def index_wiki(config, full_reindex=False):
    ...
    priority_pages = [
        p.strip().lower() for p in priority_pages_str.split(",") if p.strip()
    ]
    ...
    valid_data = [
        ...
        for chunk, chunk_id, metadata, embedding in zip(
    ...
    debug_log(
        3,
        f"Wiki indexing complete. Total documents in collection: {collection.count()}",
    )
    return True
