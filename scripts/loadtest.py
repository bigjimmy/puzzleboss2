...
class PHPEditPuzzleModule(TestModule):
    ...
    def run_single_request(self):
        ...
        if puzzle:
            resp = requests.get(
                f"{self.php_base}/editpuzzle.php?pid={puzzle['id']}", verify=False
            )
            resp.raise_for_status()
...
class APISolverAssignmentModule(TestModule):
    ...
    def run_single_request(self):
        ...
        current_solver_names = [
            s.strip() for s in current_solvers.split(",") if s.strip()
        ]
        ...
        if (
            alltime_count >= self.MAX_ALLTIME_SOLVERS
            or current_count >= self.MAX_CURRENT_SOLVERS
        ):
            ...
            if current_solver_names:
                ...
                for s in self.hunt_state.solvers:
                    if s.get("name") == solver_to_remove:
                        resp = requests.post(
                            f"{self.api_base}/solvers/{s['id']}/puzz", json={"puzz": ""}
                        )
                        resp.raise_for_status()
                        break
        else:
            # Assign the solver
            resp = requests.post(
                f"{self.api_base}/solvers/{solver_id}/puzz", json={"puzz": puzzle_id}
            )
            resp.raise_for_status()
...
class APIStatusChangingModule(TestModule):
    ...
    def run_single_request(self):
        ...
        resp = requests.post(
            f"{self.api_base}/puzzles/{puzzle['id']}/status", json={"status": status}
        )
        resp.raise_for_status()
...
class APILocationUpdatingModule(TestModule):
    ...
    def run_single_request(self):
        ...
        resp = requests.post(
            f"{self.api_base}/puzzles/{puzzle['id']}/xyzloc", json={"xyzloc": location}
        )
        resp.raise_for_status()
...
class LoadTestRunner:
    ...
    def run(self, skip_setup: bool = False):
        ...
        for module in self.modules:
            print(
                f"  - {module.name}: {module.config.threads} threads, "
    ...
    def print_results(self):
        ...
        for module in self.modules:
            ...
            print(
                f"  Errors:             {results['error_count']} ({results['error_rate']:.2f}%)"
            )
            ...
            print(
                f"  Latency (min/max):  {results['min_latency']*1000:.1f}ms / {results['max_latency']*1000:.1f}ms"
            )
            print()
...
def main():
    ...
    parser.add_argument(
        "--generate-config", action="store_true", help="Generate a default config file"
    )
    ...
    if config_path:
        ...
        print(
            "  Create scripts/loadtest_config.yaml from scripts/loadtest_config-EXAMPLE.yaml"
        )
